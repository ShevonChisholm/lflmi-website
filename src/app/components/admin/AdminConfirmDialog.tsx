...
export function AdminConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-end justify-center overflow-y-auto bg-[#04183a]/60 p-0 backdrop-blur-sm sm:items-center sm:p-6">
      <div className="w-full max-w-md rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-6">
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <button type="button" disabled={busy} onClick={onConfirm} className="flex items-center justify-center gap-2 rounded-xl bg-[#D7261E] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60">{busy && <Loader2 size={14} className="animate-spin" />}Delete</button>
        </div>
      </div>
    </div>
  );
